...
class PaClock2016TK(Tk):
	def __init__(self):
		Tk.__init__(self, None)

		self.m_nClientWidth, self.m_nClientHeight = (600, 450)
		self.m_isRunning = True
		self.m_nClockRadius = 0
		m_nTimerInterval = 0.1

		self.title('PaClock2016TK.pyw')

		self.objCanvas = Canvas(self, width=self.m_nClientWidth, height=self.m_nClientHeight, bg='#FFFFFF', bd=0)
		self.objCanvas.pack(expand=YES, fill=BOTH)

		#init interface items
		self.objLabel = self.objCanvas.create_text(10, 10, text='0000-00-00 00:00:00', anchor='w')

		self.A_rulers = [None for i in xrange(60)]
		self.A_texts = [None for i in xrange(12)]

		self.imgFace = PhotoImage(data=image1_data)
		self.objImage = self.objCanvas.create_image(0, 0,anchor=NW, image=self.imgFace)

		self.objCircle1 = self.objCanvas.create_oval(0, 0, 1, 1, width=7)
		self.objCircle2 = self.objCanvas.create_oval(0, 0, 1, 1, width=7)
		for i in xrange(60):
			m_nWidth1 = 3 if (i % 5) else 5
			self.A_rulers[i] = self.objCanvas.create_line(0, 0, 1, 1, fill=conf__strColor, width=m_nWidth1)
		self.A_hands = [
			self.objCanvas.create_line(0, 0, 1, 1, fill=conf__strColor, width=3),
			self.objCanvas.create_line(0, 0, 1, 1, fill=conf__strColor, width=5),
			self.objCanvas.create_line(0, 0, 1, 1, fill=conf__strColor, width=7)]
		#font1 = tkFont.Font(family='Arial Black',size=24)
		for i in xrange(12):
			#self.A_texts[i] = self.objCanvas.create_text(0, 0, text=i, fill='#57B777', font=font1)
			self.A_texts[i] = self.objCanvas.create_text(0, 0, text=i, fill='#57B777', font=('Arial Black', 24))
	...
	def DoInit(self):
		...
		m_nClockDiameter = self.m_nClockRadius + self.m_nClockRadius

		cx,cy = (self.m_nClientWidth//2, self.m_nClientHeight//2)
		m_nStartX = (self.m_nClientWidth - m_nClockDiameter) // 2
		m_nStartY = (self.m_nClientHeight - m_nClockDiameter) // 2

		self.objCanvas.coords(self.objImage, (cx - 100, cy - 125))

		coords1 = m_nStartX+3, m_nStartY+3, m_nStartX+m_nClockDiameter-7, m_nStartY+m_nClockDiameter-7
		self.objCanvas.coords(self.objCircle1, coords1)
		coords1 = cx-5, cy-5, cx+10, cy+10
		self.objCanvas.coords(self.objCircle2, coords1)
		...
		for i in range(60):
			...
			if i % 5 == 0:
				...
				self.objCanvas.coords(self.A_texts[i/5], coords1)
			else:
				s1 = 0.94
			coords1 = cx + dx1, cy + dy1, cx + dx1 * s1, cy + dy1 * s1
			self.objCanvas.coords(self.A_rulers[i], coords1)
	...
	def DoPaint(self):
		...
		self.objCanvas.itemconfig(self.objLabel, text=m_strDisplay)
		...
		for i in range(3):
			...
			self.objCanvas.coords(self.A_hands[i], coords1)
